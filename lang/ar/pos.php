<?php

return [
    // Navigation - التنقل
    'dashboard' => 'لوحة التحكم',
    'pos' => 'نقطة البيع',
    'warehouse' => 'المخزن',
    'suppliers' => 'الموردون',
    'purchase_orders' => 'أوامر الشراء',
    'supplier_payments' => 'مدفوعات الموردين',
    'supplier_accounts' => 'حسابات الموردين',
    'accounting' => 'المحاسبة',
    'reports' => 'التقارير',
    'financial_reports' => 'التقارير المالية',
    'returns' => 'المرتجعات',

    // Dashboard - لوحة التحكم
    'today_sales' => 'مبيعات اليوم',
    'total_revenue' => 'إجمالي الإيرادات',
    'low_stock' => 'منتجات منخفضة المخزون',
    'out_of_stock' => 'منتجات نفذت',
    'total_products' => 'إجمالي المنتجات',
    'total_suppliers' => 'إجمالي الموردين',
    'recent_invoices' => 'آخر الفواتير',
    'recent_movements' => 'آخر حركات المخزون',
    'top_products' => 'أفضل المنتجات مبيعاً',
    'growth_vs_yesterday' => 'النمو مقارنة بالأمس',

    // POS - نقطة البيع
    'invoice_number' => 'رقم الفاتورة',
    'product_name' => 'اسم المنتج',
    'quantity' => 'الكمية',
    'unit_price' => 'سعر الوحدة',
    'subtotal' => 'المجموع الفرعي',
    'discount' => 'الخصم',
    'total' => 'الإجمالي',
    'payment_method' => 'طريقة الدفع',
    'cash' => 'نقداً',
    'card' => 'بطاقة',
    'transfer' => 'تحويل',
    'complete_sale' => 'إتمام البيع',
    'print_invoice' => 'طباعة الفاتورة',
    'scan_barcode' => 'مسح الباركود',
    'search_product' => 'بحث عن منتج',
    'add_to_cart' => 'إضافة للسلة',
    'cart_empty' => 'السلة فارغة',
    'product_not_found' => 'المنتج غير موجود',
    'invoice_not_found' => 'الفاتورة غير موجودة',
    'insufficient_stock' => 'المخزون غير كافٍ للمنتج: :name',
    'sale_completed' => 'تم البيع بنجاح',
    'sale_deduction' => 'خصم مبيعات',

    // Products - المنتجات
    'add_product' => 'إضافة منتج',
    'edit_product' => 'تعديل منتج',
    'delete_product' => 'حذف منتج',
    'barcode' => 'الباركود',
    'category' => 'التصنيف',
    'selling_price' => 'سعر البيع',
    'cost_price' => 'سعر التكلفة',
    'min_stock' => 'الحد الأدنى للمخزون',
    'current_stock' => 'المخزون الحالي',
    'add_stock' => 'إضافة مخزون',
    'stock_added' => 'تم إضافة المخزون بنجاح',
    'new_product_added' => 'إضافة منتج جديد',
    'manual_stock_add' => 'إضافة مخزون يدوية',

    // Suppliers - الموردون
    'add_supplier' => 'إضافة مورد',
    'edit_supplier' => 'تعديل مورد',
    'delete_supplier' => 'حذف مورد',
    'phone' => 'الهاتف',
    'address' => 'العنوان',
    'email' => 'البريد الإلكتروني',

    // Purchase Orders - أوامر الشراء
    'create_po' => 'إنشاء أمر شراء',
    'po_number' => 'رقم أمر الشراء',
    'order_date' => 'تاريخ الطلب',
    'expected_date' => 'التاريخ المتوقع',
    'receive_po' => 'استلام الطلب',
    'po_status_pending' => 'معلق',
    'po_status_received' => 'مستلم',
    'po_status_partial' => 'مستلم جزئياً',
    'po_status_cancelled' => 'ملغي',
    'purchase_receipt' => 'استلام أمر شراء: :po',
    'po_debt_note' => 'دين أمر شراء: :po',

    // Payments - المدفوعات
    'add_payment' => 'إضافة دفعة',
    'payment_number' => 'رقم الدفعة',
    'payment_date' => 'تاريخ الدفع',
    'amount' => 'المبلغ',

    // Accounting - المحاسبة
    'add_account' => 'إضافة حساب',
    'account_code' => 'كود الحساب',
    'account_name' => 'اسم الحساب',
    'account_type' => 'نوع الحساب',
    'asset' => 'أصول',
    'liability' => 'خصوم',
    'equity' => 'حقوق ملكية',
    'revenue' => 'إيرادات',
    'expense' => 'مصروفات',
    'journal_entry' => 'قيد يومية',
    'debit' => 'مدين',
    'credit' => 'دائن',
    'journal_unbalanced' => 'القيد غير متوازن: إجمالي المدين ≠ إجمالي الدائن',
    'account_has_dependencies' => 'لا يمكن حذف الحساب لوجود حسابات فرعية أو قيود مرتبطة به',

    // Reports - التقارير
    'start_date' => 'تاريخ البداية',
    'end_date' => 'تاريخ النهاية',
    'income_statement' => 'قائمة الدخل',
    'balance_sheet' => 'الميزانية العمومية',
    'sales_report' => 'تقرير المبيعات',
    'stock_report' => 'تقرير المخزون',
    'net_income' => 'صافي الدخل',
    'total_assets' => 'إجمالي الأصول',
    'total_liabilities' => 'إجمالي الخصوم',

    // Returns - المرتجعات
    'return_number' => 'رقم المرتجع',
    'return_reason' => 'سبب الإرجاع',
    'process_return' => 'معالجة المرتجع',
    'return_note' => 'مرتجع: :ret',
    'returnable_qty' => 'الكمية القابلة للإرجاع',

    // Common - مشترك
    'save' => 'حفظ',
    'cancel' => 'إلغاء',
    'delete' => 'حذف',
    'edit' => 'تعديل',
    'view' => 'عرض',
    'search' => 'بحث',
    'filter' => 'تصفية',
    'print' => 'طباعة',
    'export' => 'تصدير',
    'loading' => 'جارٍ التحميل...',
    'confirm_delete' => 'هل أنت متأكد من الحذف؟',
    'success' => 'تمت العملية بنجاح',
    'error' => 'حدث خطأ',
    'notes' => 'ملاحظات',
    'actions' => 'الإجراءات',
    'status' => 'الحالة',
    'date' => 'التاريخ',
    'name' => 'الاسم',
    'no_data' => 'لا توجد بيانات',
    'currency' => 'ج.م',
    'app_name' => 'نظام نقطة البيع',
    'username' => 'اسم المستخدم',
    'password' => 'كلمة المرور',
    'login' => 'تسجيل الدخول',
    'logout' => 'تسجيل الخروج',
    'welcome' => 'مرحباً',
    'language' => 'اللغة',
    'close' => 'إغلاق',
    'thank_you' => 'شكرًا',
    'cashier' => 'أمين الصندوق',
    'Select a supplier to view their account' => 'اختر موردًا لعرض حسابه',
    'total_stock_value' => 'القيمة الإجمالية للمخزون',

    // Settings - الإعدادات (new keys)
    'settings' => 'الإعدادات',
    'settings_saved' => 'تم حفظ الإعدادات بنجاح',
    'general_settings' => 'الإعدادات العامة',
    'tax_settings' => 'إعدادات الضريبة',
    'invoice_settings' => 'إعدادات الفاتورة',
    'pos_settings' => 'إعدادات نقطة البيع',
    'store_name' => 'اسم المتجر',
    'store_address' => 'عنوان المتجر',
    'store_phone' => 'هاتف المتجر',
    'store_email' => 'البريد الإلكتروني',
    'currency' => 'العملة',
    'currency_symbol' => 'رمز العملة',
    'default_language' => 'اللغة الافتراضية',
    'tax_enabled' => 'تفعيل الضريبة',
    'tax_rate' => 'نسبة الضريبة',
    'tax_name' => 'اسم الضريبة',
    'tax_inclusive' => 'السعر شامل الضريبة',
    'tax_number' => 'الرقم الضريبي',
    'tax_amount' => 'قيمة الضريبة',
    'before_tax' => 'قبل الضريبة',
    'after_tax' => 'بعد الضريبة',
    'invoice_prefix' => 'بادئة رقم الفاتورة',
    'invoice_footer' => 'تذييل الفاتورة',
    'show_tax_invoice' => 'إظهار الضريبة في الفاتورة',
    'auto_print' => 'طباعة تلقائية',
    'pos_sound' => 'صوت المسح',
    'low_stock_alert' => 'تنبيه المخزون المنخفض',
    'allow_negative_stock' => 'السماح بمخزون سالب',
    'default_payment' => 'طريقة الدفع الافتراضية',
    'returns_report' => 'تقرير المرتجعات',
    'total_returned' => 'إجمالي المرتجعات',
    'return_date' => 'تاريخ الإرجاع',
    'top_returned_products' => 'أكثر المنتجات إرجاعاً',
    'returns_count' => 'عدد المرتجعات',
    'return_quantity_exceeded' => 'الكمية المرتجعة للمنتج ":name" تتجاوز الحد المسموح به (:max)',
    'return_quantity_invalid'  => 'كمية غير صالحة للمنتج ":name"',
    'unauthorized'             => 'ليس لديك صلاحية للوصول إلى هذه الصفحة',
    'Account Statement' => 'كشف الحساب',
    'Account' => 'الحساب',
    'Returns Report' => 'تقرير المرتجعات',
    'total_amount' => 'المبلغ الإجمالي',
    'select_date_range' => 'حدد نطاق التاريخ للتحميل',
    'returns_list' => 'قائمة المرتجعات',
    'Customer' =>'العميل',
    'reason'=>'السبب',
    'total_returned_value' => 'إجمالي قيمة المرتجعات',
    'total_returns_count' => 'إجمالي عدد المرتجعات',
    'avg_return_value' => 'متوسط قيمة المرتجع',
    'completed' => 'مكتمل',
    'cancelled' => 'ملغي',
    'qty' => 'الكمية',
    'final' => 'السعر النهائي',
    'stock_value' => 'قيمة المخزون',
    // Roles & Permissions - الأدوار والصلاحيات
    'roles_permissions' => 'الأدوار والصلاحيات',
    'roles' => 'الأدوار',
    'permissions' => 'الصلاحيات',
    'role_name' => 'اسم الدور',
    'create_role' => 'إنشاء دور',
    'edit_role' => 'تعديل دور',
    'delete_role' => 'حذف دور',
    'role_created' => 'تم إنشاء الدور بنجاح',
    'role_updated' => 'تم تحديث الدور بنجاح',
    'role_deleted' => 'تم حذف الدور بنجاح',
    'permissions_updated' => 'تم تحديث الصلاحيات بنجاح',
    'role_assigned' => 'تم تعيين الدور بنجاح',
    'assign_role' => 'تعيين دور',
    'select_role' => 'اختر دور',
    'select_user' => 'اختر مستخدم',
    'users' => 'المستخدمين',
    'guard_name' => 'اسم الحارس',
    'no_roles' => 'لا توجد أدوار',
    'no_permissions' => 'لا توجد صلاحيات',
    'current_roles' => 'الأدوار الحالية',
    'Generate' => 'إنشاء',


];