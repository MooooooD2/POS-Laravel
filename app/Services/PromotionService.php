    ...
    public function apply(array $items, float $orderTotal): array
    {
        ...
        foreach ($promotions as $promo) {
            ...
            $promoItems = collect($items)->filter(function ($item) use ($promo) {
                if ($promo->product_id && $item['product_id'] != $promo->product_id) return false;
                if ($promo->product_category && ($item['product']->category ?? '') !== $promo->product_category) return false;
                return true;
            });
            ...
            switch ($promo->type) {
                ...
                case 'buy_x_get_y':
                    if ($promo->buy_qty <= 0 || $promo->get_qty <= 0) break;
