...
class ThermalPrinterService
{
    ...
    public function printInvoice(Invoice $invoice, ?Printer $printer = null): array
    {
        ...
        if (!$printer) return $this->fallbackBrowserPrint($invoice, 'invoice');
    ...
    {
        $printer = $printer ?? $this->getDefaultPrinter();
        if (!$printer) return $this->fallbackBrowserPrint($return, 'return');
    ...
    {
        $printer = $printer ?? $this->getDefaultPrinter();
        if (!$printer) return $this->fallbackBrowserPrint($session, 'shift_report');
    ...
    private function renderSection(EscposDriver $driver, array $section): void
    {
        ...
        foreach ($section['lines'] ?? [] as $line) {
            ...
            if ($isBold)   $driver->setBold(true);
            if ($isDouble) $driver->setDoubleHeight(true);
            ...
            if ($isBold)   $driver->setBold(false);
            if ($isDouble) $driver->setDoubleHeight(false);
        }
    }
