...
Route::middleware(['auth', 'throttle:60,1'])->prefix('hr')->name('api.hr.')->group(function () {
    ...
    Route::middleware('permission:manage_settings')->group(function () {
        ...
        Route::get('/employees', function (\Illuminate\Http\Request $req) {
            $q = \DB::table('users')
                ...
                ->select(
                    'users.id', 'users.full_name', 'users.username', 'users.role',
                    'users.is_active', 'users.branch_id',
            ...
            if ($req->branch_id) $q->where('users.branch_id', $req->branch_id);
            if ($req->role)      $q->where('users.role', $req->role);
            if ($req->status === 'active')   $q->where('users.is_active', true);
            if ($req->status === 'inactive') $q->where('users.is_active', false);
            if ($req->search)    $q->where(fn($w) => $w->where('users.full_name', 'like', '%'.$req->search.'%')
    ...
    Route::get('/leaves', function (\Illuminate\Http\Request $req) {
        ...
        if (!$isAdmin) $q->where('leave_requests.user_id', auth()->id());
        if ($req->status) $q->where('leave_requests.status', $req->status);
        if ($req->type)   $q->where('leave_requests.leave_type', $req->type);
...
Route::middleware(['auth', 'throttle:60,1'])->prefix('currencies')->name('api.currencies.')->group(function () {
    ...
    Route::middleware('permission:manage_settings')->group(function () {
        Route::post('/', function (\Illuminate\Http\Request $req) {
            ...
            if ($req->boolean('is_base')) \DB::table('currencies')->update(['is_base' => false]);
...
Route::middleware(['auth', 'permission:view_reports', 'throttle:30,1'])->prefix('franchise')->name('api.franchise.')->group(function () {
    ...
    Route::get('/statements', function (\Illuminate\Http\Request $req) {
        ...
        if ($req->year)   $q->where('period_year', $req->year);
        if ($req->month)  $q->where('period_month', $req->month);
