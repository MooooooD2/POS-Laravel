...
Route::middleware(['auth'])->group(function () {
    ...
    // ── Warehouse / Products (users with permission) ───────────────────────────────
    Route::middleware(['permission:view_warehouse'])->group(function () {
        ...
        Route::post('/api/products', [ProductController::class, 'store'])->name('products.store')->middleware('permission:add_product');
        Route::put('/api/products/{product}', [ProductController::class, 'update'])->name('products.update')->middleware('permission:edit_product');
        Route::delete('/api/products/{product}', [ProductController::class, 'destroy'])->name('products.destroy')->middleware('permission:delete_product');
        Route::post('/api/products/{product}/add-stock', [ProductController::class, 'addStock'])->name('products.add-stock')->middleware('permission:add_stock');

        // Suppliers
        Route::get('/suppliers', [SupplierController::class, 'index'])->name('suppliers');
        Route::get('/api/suppliers', [SupplierController::class, 'all'])->name('suppliers.all');
        Route::post('/api/suppliers', [SupplierController::class, 'store'])->name('suppliers.store')->middleware('permission:add_supplier');
        Route::put('/api/suppliers/{supplier}', [SupplierController::class, 'update'])->name('suppliers.update')->middleware('permission:edit_supplier');
        Route::delete('/api/suppliers/{supplier}', [SupplierController::class, 'destroy'])->name('suppliers.destroy')->middleware('permission:delete_supplier');
        ...
        Route::post('/api/purchase-orders', [PurchaseOrderController::class, 'store'])->name('purchase-orders.store')->middleware('permission:create_purchase_order');
        Route::post('/api/purchase-orders/{purchaseOrder}/receive', [PurchaseOrderController::class, 'receive'])->name('purchase-orders.receive')->middleware('permission:receive_purchase_order');
        ...
        Route::post('/api/supplier-payments', [SupplierPaymentController::class, 'store'])->name('supplier-payments.store')->middleware('permission:create_supplier_payment');

        // Supplier Accounts (read)
    ...
    // ── Accounting & Reports (admin only via permissions) ────────────────────────────────────
    Route::middleware(['permission:view_accounting'])->group(function () {
        ...
        Route::post('/api/accounts', [AccountingController::class, 'storeAccount'])->name('accounts.store')->middleware('permission:manage_accounts');
        Route::put('/api/accounts/{account}', [AccountingController::class, 'updateAccount'])->name('accounts.update')->middleware('permission:manage_accounts');
        Route::delete('/api/accounts/{account}', [AccountingController::class, 'destroyAccount'])->name('accounts.destroy')->middleware('permission:manage_accounts');
        Route::get('/api/journal-entries', [AccountingController::class, 'allJournalEntries'])->name('journal-entries.all');
        Route::post('/api/journal-entries', [AccountingController::class, 'storeJournalEntry'])->name('journal-entries.store')->middleware('permission:create_journal_entry');
        ...
        Route::post('/api/settings', [SettingController::class, 'update'])->name('settings.update')->middleware('permission:update_settings');
        Route::get('/api/settings/group/{group}', [SettingController::class, 'group'])->name('settings.group');
    });

    // ── Reports (users with permission) ────────────────────────────────────
    ...
    // ── Roles & Permissions (admin only via permissions) ────────────────────────────────────
